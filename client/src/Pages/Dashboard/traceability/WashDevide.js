...
const WashDevide = () => {
  ...
  const pending = await isNLotPending(lot.newlot_nLot);
  if (pending) {
    alert(t("lotPending"));
    return;
  }
  ...
  const totalVolume = Math.round(
    selectedLots.reduce((acc, lot) => acc + lot.volume, 0),
  );
  ...
  const handleSubmit = (e) => {
    ...
    const initial = {};
    previewLots.forEach((l) => {
      initial[l.key] = patioOptions[0]?.name || "";
    });
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      await axios.post(
        "http://localhost:5000/api/patio_prevnlot",
        patioPrevNlotData,
      );
      await Promise.all(
        selectedLots.map((lot) =>
      ...
      case 0:
        return (
          <div>
            ...
            <label>
              {t("matureGreen")}: <strong>{matureGreen}%</strong> (
              {matureGreenVolume.toLocaleString("it-IT")} L)
              <input
                type="range"
                min="0"
                max="100"
              ...
              />
              {t("types.Dry")}: <strong>{100 - matureGreen}%</strong> (
              {dryVolume.toLocaleString("it-IT")} L)
            </label>
          </div>
        );
      ...
      case 1:
        return (
          <div>
            <p className="text-muted" style={{ marginBottom: "0.75rem" }}>
              {t("types.Dry")}:{" "}
              <strong>{dryVolume.toLocaleString("it-IT")} L</strong>
            </p>
            <label
              style={{
                display: "flex",
            ...
            {isFiltered && (
              <div className="lot-ranges" style={{ padding: "1rem" }}>
                <label>
                  {t("types.BigDry")}: <strong>{dryFiltered}%</strong> (
                  {bigDryVolume.toLocaleString("it-IT")} L)
                  <input
                    type="range"
                    min="0"
                    max="100"
                  ...
                  />
                  {t("types.Dry")}: <strong>{100 - dryFiltered}%</strong> (
                  {dryAfterFilter.toLocaleString("it-IT")} L)
                </label>
              </div>
            )}
          </div>
      ...
      case 2:
        return (
          <div>
            <p className="text-muted" style={{ marginBottom: "0.75rem" }}>
              {t("matureGreen")}:{" "}
              <strong>{matureGreenVolume.toLocaleString("it-IT")} L</strong>
            </p>
            <label
              style={{
                display: "flex",
            ...
            {isDepulped && (
              <div className="lot-ranges" style={{ padding: "1rem" }}>
                <label>
                  {t("types.CD")}: <strong>{cdDepulped}%</strong> (
                  {cdVolume.toLocaleString("it-IT")} L)
                  <input
                    type="range"
                    min="0"
                    max="100"
                  ...
                  />
                  {t("types.Green")}: <strong>{100 - cdDepulped}%</strong> (
                  {greenVolume.toLocaleString("it-IT")} L)
                </label>
                <div
                  style={{
                    marginTop: "0.75rem",
              ...
              >
                <div>
                  ...
                  {lot.type === "CD" && (lot.demucil || lot.centrifug) && (
                    ...
                    >
                      {[
                        lot.demucil && "Desmucil.",
                        lot.centrifug && t("centrifuga"),
                      ]
                        .filter(Boolean)
                        .join(" + ")}
                    </span>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <label>
          ...
          {minDate && !dateError && selectedLots.length > 0 && (
            ...
            >
              {t("minDateHint")}:{" "}
              {new Date(minDate).toLocaleDateString("it-IT")}
            </span>
          )}
        </label>
